...
const ListaDeTarefas = () => {
  ...
  const filtraTarefas = () => {
    ...
    if (termo !== undefined) {
      tarefasFiltradas = itens.filter((item) => item.titulo.toLowerCase().search(termo.toLowerCase()) >= 0)
  ...
  const exigeResultadoFiltragem = (quantidade: number) => {
    let mensagem = ''
  ...
  const mensagem = exigeResultadoFiltragem(tarefas.length)
